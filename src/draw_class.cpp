...
void Draw::drawElipse(cv::Mat &image, Node observer)
{
    ...
    cv::ellipse(image,                                                     // cv::InputOutputArray image,
...
{
    cv::Point2d temp_opencv_point_x_y = cartesianPointToOpenCVPoint(node.cartesian_x_y_point);
    cv::circle(image, temp_opencv_point_x_y, node.getVehicleSize(), node.nodeColorPalet.vehicle_color, cv::FILLED, 8, 0);
    cv::circle(image, temp_opencv_point_x_y, node.getInner(), node.nodeColorPalet.inner__color, 2, 8, 0);
    cv::circle(image, temp_opencv_point_x_y, node.getMeasurmentError(), node.nodeColorPalet.measurment_error_color, 2, 8, 0);
}

void Draw::drawFrame(cv::Mat &image, std::vector<Node> node_list)
{
    drawNode(image, node_list.at(0));
    drawNode(image, node_list.at(1));
    drawConnectingLine(image, node_list.at(0), node_list.at(1));
    generateText(node_list);
    openCVdrawTextOnScreen(image, node_list);
    openCVdrawAxis(image, node_list.at(0));
    openCVdrawAxis(image, node_list.at(1));
    drawElipse(image, node_list.at(0));
}
...
void Draw::drawConnectingLine(cv::Mat &image, Node observer, Node target)
{
    ...
    cv::line(image,
             ...
             cv::LineTypes::LINE_4,
             0);
    this->middle_of_angle_line = cv::Point2d((temp_observer_cartesian_x_y_point.x + temp_target_cartesian_x_y_point.x) / 2,
